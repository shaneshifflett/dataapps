/*
 * Sliding Tabs 1.0.6 - jQuery Plugin
	...
	$.fn.slideTabs = function(options) {
		...
		this.each(function() {				
			var container = $(this),
				...
				activeElem = tabs.children("li").find("."+opts.tabActiveClass);						
									
			// Hide the overflowing content
			content.css({overflow: "hidden"});
			
			if (opts.autoHeight == true) {
				// Set the content container's height to auto
				content.parent().css({height: "auto"});
			}
			
			// Set the first tab element to 'active' if none is found
			if (!activeElem.length) {			
				activeElem = tabs.find("a:first").addClass(opts.tabActiveClass);				
			}						
			
			// Show the active tab's content													
			content.children(activeElem.attr("href")).addClass(opts.viewActiveClass);																		
			...
			switch(opts.contentAnim) {
				case "slideLeft": 										
					content.children("div:not(."+opts.viewActiveClass+")").css({left: content.parent("div").outerWidth(true)});
					break;
				case "slideRight":					
					content.children("div:not(."+opts.viewActiveClass+")").css({left: -content.parent("div").outerWidth(true)});  
					break;
				case "slideUp":					
					content.children("div:not(."+opts.viewActiveClass+")").css({display: "none", top: content.parent("div").outerHeight(true)});
					break;
				case "slideDown":					
					content.children("div:not(."+opts.viewActiveClass+")").css({display: "none", top: -content.parent("div").outerHeight(true)});
					break;
				default:					
					content.children("div:not(."+opts.viewActiveClass+")").hide();
			}
			...
			var contAnim = "#"+container.attr("id")+" :animated",
				tab, elem, elemD, elemP = 0, prevElem, prevElemPos = 0, elemHidden = 0, activeView, view;						
			...
			function tabClick(tab) {				
				...
				if($(contAnim).length || tab.hasClass(opts.tabActiveClass))
					return false;								
				...
				tabs.children("li").find("a."+opts.tabActiveClass).removeClass(opts.tabActiveClass);					
				tab.addClass(opts.tabActiveClass);
				...
				activeView = content.children("div."+opts.viewActiveClass);				
				view = content.children("div#"+tab.attr("hash"));																							
								
				if (opts.autoHeight == true) {
					// Set the content's height
					if (opts.autoHeightTime > 0) {
						content.animate({height: view.height()+"px"}, opts.autoHeightTime);
					} else {
						content.css({height: view.height()+"px"});
					}					
				}
				...
				activeView.removeClass(opts.viewActiveClass);													
				view.addClass(opts.viewActiveClass);								
			};						
			...
			function slideDown() {
				activeView.animate({top: view.outerHeight(true)+"px"}, opts.contentAnimTime, opts.contentEasing);								
			...
			var tabsAnim = "#"+container.attr("id")+" "+opts.tabsList+":animated";
			...
			// Bind the tabs click event
			tabs.find("li a."+opts.tabClass).bind("click", function() {
				tabClick(this);
				return false;
			});							
			...
			container.find("ul.customize li a#add").click(function() {									
				...
				if (opts.orientation == "horizontal") {							
					...
					tabs.append('<li><a href="#v_content_'+length+'" id="tab_'+length+'" class="tab">Vertical Tab #'+length+'<span>Lorem ipsum dolor sit amet</span></a></li>');	
					...
					content.append('<div id="v_content_'+length+'" class="tab_view"><h2>Tab number #'+length+'</h2><div class="text">'+content.children("div#v_content_1").find("div.text").html()+'</div></div>');							
				}
				
				// Bind click event to the appended tab
				tabs.find("li a:last").bind("click", function() {
					tabClick(this);
					return false;
				});				
				...
				switch(opts.contentAnim) {
					...
					case "slideRight":
						...
						break;
					case "slideUp":
						appended.css({display: "none", position: "absolute", top: content.parent("div").outerHeight(true), left: "0px"});
						break;
					case "slideDown":
						appended.css({display: "none", position: "absolute", top: -content.parent("div").outerHeight(true), left: "0px"});	
						break;
			...
			container.find("ul.customize li a#remove").click(function() {						
				...
				if (elem.children("a").hasClass(opts.tabActiveClass)) {											
					var prevLink = elem.prev().children("a"),						
						tabLink = prevLink.attr("hash");
														
					prevLink.addClass(opts.tabActiveClass);
								
					// Show the previous tab's content												
					content.children(tabLink).css({top: "0px", left: "0px", display: "block"}).addClass(opts.viewActiveClass);
				}
			...
			optsBox.find("a#save").click(function() {																																				
				...
				if (optsBox.find("input#orientation").val() == "horizontal") {
					...
					opts.tabsScroll = optsBox.find('input[name="scroll"]:checked').val();
				} else {
					...
					opts.tabsScroll = optsBox.find('input[name="v_scroll"]:checked').val();
				} 								 													
				...
				switch(opts.contentAnim) {
					case "slideLeft": 											
						content.children("div:not(."+opts.viewActiveClass+")").css({top: "0px", left: content.parent("div").outerWidth(true)});
						break;
					case "slideRight":						
						content.children("div:not(."+opts.viewActiveClass+")").css({top: "0px", left: -content.parent("div").outerWidth(true)});  
						break;
					case "slideUp":						
						content.children("div:not(."+opts.viewActiveClass+")").css({display: "none", top: content.parent("div").outerHeight(true), left: "0px"});
						break;
					case "slideDown":						
						content.children("div:not(."+opts.viewActiveClass+")").css({display: "none", top: -content.parent("div").outerHeight(true), left: "0px"});
						break;
					default:						
						content.children("div:not(."+opts.viewActiveClass+")").css({top: "0px", left: "0px", display: "none"});
				}										
